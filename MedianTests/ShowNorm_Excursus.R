...
graphic_tbl <- bind_rows(tibble(Name="Normalverteilung", Color=colors[1], Linetype=1, Size=1, Display="Normalverteilung", Type="Dichte"), 
...
my_mean <- 0
my_median <- 0
df <- bind_cols(x=x,y=y,Type="Normalverteilung")


main <- ggplot(df, aes(x=x,y=y,color=Type,linetype=Type, size=Type)) +
  geom_area(fill=colors[1], alpha=0.2) +
  ggtitle("Dichte der Normalverteilung, Mittelwert=0, Standardabweichung=1") +
  geom_segment(aes(x=my_mean,xend=my_mean,y=0,yend=0.4, color="Mittelwert", size="Mittelwert",linetype="Mittelwert")) +
  annotate(geom="text", x=my_mean, y=0.41, label=sprintf("%.2f",my_mean),color=graphic_tbl[graphic_tbl$Name=="Mittelwert","Color"], fontface=2) +
  geom_segment(aes(x=my_median,xend=my_median,y=0,yend=0.4, color="Median", size="Median",linetype="Median")) +
  annotate(geom="text", x=my_median, y=0.42, label=sprintf("%.2f",my_median),color=graphic_tbl[graphic_tbl$Name=="Median","Color"], fontface=2) +
